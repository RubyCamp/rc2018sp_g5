...
#      @player.update(@playershots,@playershots2,@light_data,Ligh_f.main(open_light,close_light,@board.analog_read(0)),Distance_senser.main(@senser_all.distance_value_getter))#プレイヤーの弾の描画
      @player.update(@playershots,@playershots2,@light_data ,@senser.light(open_light,close_light,@senser_all.light_now_getter) ,Distance_senser.main(@senser_all.distance_value_getter) ,@senser.sw_puch(@senser_all.sw_now_getter) ,@senser.oto_puch(@senser_all.oto_now_getter))#プレイヤーの弾の描画
